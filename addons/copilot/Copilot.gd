...
@onready var model_select = %Model
...
@onready var deployment_name: LineEdit = %Deployment
@onready var endpoint: LineEdit = %Endpoint
@onready var apiversion: LineEdit = %Apiversion
...
var openai_api_key
var cur_model
var azure_deployment: String
var azure_endpoint: String
var azure_apiversion: String
...
func get_llm():
    ...
    if cur_model.begins_with('azure'):
        llm._azure_end_point(azure_endpoint)
        llm._azure_deployment_name(azure_deployment)
        llm._azure_api_version(azure_apiversion)
    return llm
...
func _on_model_selected(index):
    ...
    if cur_model.begins_with("azure"):
        deployment_name.visible = true
        endpoint.visible = true
        apiversion.visible = true
    else:
        deployment_name.visible = false
...
func store_config():
    ...
    config.set_value("azure", "apiversion", azure_apiversion)
...
func load_config():
    ...
    azure_deployment = config.get_value("azure", "deployment", "")
    deployment_name.text = azure_deployment
    azure_endpoint= config.get_value("azure", "endpoint", "")
    endpoint.text = azure_endpoint
    azure_apiversion = config.get_value("azure", "apiversion", "")
    apiversion.text = azure_apiversion
...
func _on_deployment_text_changed(new_text: String) -> void:
    azure_deployment = new_text
    store_config()


func _on_endpoint_text_changed(new_text: String) -> void:
    azure_endpoint = new_text
    store_config()


func _on_apiversion_text_changed(new_text: String) -> void:
    azure_apiversion = new_text
    store_config()
